            ...
            switch (key)
            {
                case 1:
                    products = _context.Products.Include(p => p.ProductImages).OrderByDescending(p => p.Price).Take(3).ToList();
                      break;
                case 2:
                    products = _context.Products.Include(p => p.ProductImages).OrderBy(p => p.Price).Take(3).ToList();
                    break;
                case 3:
                    products = _context.Products.Include(p => p.ProductImages).OrderByDescending(p => p.Id).Take(3).ToList();
                    break;
